...
- **計畫可以隨時停止、重新設定**：模擬計畫執行到一半也能停止，畫面上「已完成」欄位會保留目前累積的距離，可以參考這個數字，改用「自訂距離與時間」精準補足剩下差的公里數，不用整個重跑。
- **自訂距離與時間，精準補距離**：孵蛋距離只差一點點時特別好用——直接輸入還缺的公里數與想花的時間，不用被固定的預設距離卡綁住。
- **孵蛋減半活動開關**：Pokémon GO 有孵蛋減半活動時打開，所有距離需求自動減半，活動期間特別實用。
- **完成提醒（鬧鈴／震動）**：計畫完成時可以響鈴通知，手機設定靜音時會自動改成震動；也可以在設定頁強制永遠只震動，不管手機鈴聲模式為何。
